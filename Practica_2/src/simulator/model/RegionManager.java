...
public class RegionManager implements AnimalMapView {
	...
	public void unregister_animal(Animal a) { // COMPROBAR
		Region current_region = _animal_region.get(a); //find_animal_region(a);
	...
	public List<Animal> get_animals_in_range(Animal a, Predicate<Animal> filter) {
		...
		for (int i = 0; i < this._rows; i++) {
			for (int j = 0; j < this._columns; j++) {
				aux_list = _regions[i][j].getAnimals();
				for (Animal animal_aux : aux_list) {
					distance = a.get_position().distanceTo(animal_aux.get_position());
					if (distance <= a.get_sight_range() && filter.test(animal_aux) && a != animal_aux) {
	...
	public Iterator<RegionData> iterator() {
		return new Iterator<RegionData>() {
			...
			/*
			 * INICIALIZAMOS UNA i PARA USAR COMO INDICE Y UN BOOLEANO PARA VER SI EXISTE
			 * SIGUIENTE Y LO IGUALAMOS A TRUE YA QUE DE PRIMERAS SUPONEMOS QUE HAY
			 * SIGUEINTE
			 */
			int i = 0;
			int j = 0;
			boolean existsNext = true;

			@Override
			public boolean hasNext() {
				// SI i ES IGUAL A SIZE ES QUE NO HAY SIGUEINTE
				if (i == RegionManager.this._rows || j == RegionManager.this._columns)
					existsNext = false;
			...
			public RegionData next() {
				RegionData r = new RegionData(i, j, RegionManager.this._regions[i][j]); // SACAMOS LA REGION CORRESPONDIENTE A i
				j++;
				if (j == RegionManager.this._columns) {
					i++;
					j = 0;
				}
				return r;
